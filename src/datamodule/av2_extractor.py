...
class Av2Extractor:
    ...
    def process(self, raw_path: str, agent_id=None):
        ...
        x_velocity_vector = torch.zeros(num_nodes, 110, 2, dtype=torch.float)
        ...
        for actor_id, actor_df in df.groupby("track_id"):
            ...
            velocity_vector = torch.from_numpy(
                np.stack(
                    [actor_df["velocity_x"].values, actor_df["velocity_y"].values],
                    axis=-1,
                )
            ).float()
            ...
            x_velocity_vector[node_idx, node_steps, :2] = velocity_vector
        ...
        return {
            ...
            "x_velocity_vector": x_velocity_vector,
