...
class Trainer(pl.LightningModule):
    ...
    def cal_loss(self, out, data, tag=''):
        ...
        if dense_predict is not None:
            dense_predict_reg_loss = F.smooth_l1_loss(dense_predict, y)
        else:
            dense_predict_reg_loss = 0
        ...
        loss = loss + y_hat_new_reg_loss + dense_predict_reg_loss + pi_new_cls_loss
        disp_dict = {
        ...
        if dense_predict is not None:
            disp_dict[f"{tag}dense_predict_reg_loss"] = dense_predict_reg_loss.item()
        ...
        dense_predict_polar = out.get("dense_predict_polar", None)
        if dense_predict_polar is not None:
            y_polar = cartesian_to_polar(y)
            dense_predict_polar_reg_loss = F.smooth_l1_loss(dense_predict_polar[..., 0], y_polar[..., 0]) + \
                                           F.smooth_l1_loss(dense_predict_polar[..., 1], y_polar[..., 1])
            loss = loss + dense_predict_polar_reg_loss
            disp_dict[f"{tag}dense_predict_polar_reg_loss"] = dense_predict_polar_reg_loss.item()
...
class StreamTrainer(Trainer):
    ...
    def test_step(self, data, batch_idx) -> None:
        ...
        if self.pre_ensemble:
            ...
            # data[-1]["origin"]
            # data[-1]["theta"]
            # all_outs[-1]["y_hat"] torch.Size([B, 6, 60, 2])
            # all_outs[-1]["pi"] torch.Size([B, 6])
            ensemble_num = 'en_888'
            ...
            for i in range(batch):
                ...
                with open(f'ensemble/save_for_en/{ensemble_num}/{scenario_id}_with_{track_id}.pkl', 'wb') as outp: 
